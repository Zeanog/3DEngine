...
public:
	virtual void Set(const rapidjson::Value& src, Byte* dest, UInt64 size) const = 0;
};
...
public:
	...
	virtual void Set(const TValue& value, rapidjson::Value& outValue) const = 0;
...
class ValueParser<List<_TElemValue>> : public AValueParser<List<_TElemValue>> {
	INHERITED_CLASS_TYPEDEFS(ValueParser, AValueParser<List<_TElemValue>>)
...
public:
	...
	virtual void Set(const TValue& value, rapidjson::Value& outValue) const override {
		...
		FOREACH_CONST(iter, value) {
			rapidjson::Value elemValue{};
			parser->Set(*iter, elemValue);
			outValue.PushBack(elemValue, rapidjson::Document::AllocatorType());
		}
	}
...
class ValueParser<Map<StaticString, _TMapValue>> : public AValueParser<Map<StaticString, _TMapValue>> {
	typedef Map<StaticString, _TMapValue> TTypeMap;
	INHERITED_CLASS_TYPEDEFS(ValueParser, AValueParser<TTypeMap>)
	SINGLETON_DECLARATIONS(ValueParser) {
	}

public:
	typedef TSuper::TValue	TValue;
	typedef _TMapValue		TMapValue;
...
public:
	virtual void Get(const rapidjson::Value& value, TValue& outValue) const override {
		...
		FOREACH_MEMBER(iter, value) {
			...
			outValue.Add(iter->name.GetString(), std::move(elem));
		}
	}
...
public:
	...
	virtual void Set(const TValue& value, rapidjson::Value& outValue) const override {
		outValue.SetDouble((double)value);
	}
};
...
public:
	...
	virtual void Set(const TValue& value, rapidjson::Value& outValue) const override {
		outValue.SetString(value.CStr(), value.Length());
	}
};
